...
const roleLabels = { student: 'Aluno', teacher: 'Professor', admin: 'Admin' };
const roleBadges = { student: 'badge-student', teacher: 'badge-teacher', admin: 'badge-admin' };
          ...
          <tbody>
            {filtered.map(user => (
              <tr key={user.id}>
                ...
                <td><span className={`badge ${roleBadges[user.role]}`}>{roleLabels[user.role]}</span></td>
                <td><span className={`badge ${statusBadges[user.status]}`}>{statusLabels[user.status]}</span></td>
                <td style={{ color: '#6b778c', fontSize: 13 }}>{user.date}</td>
                <td>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {user.status === 'pending' && (
                      <>
                        <button className="action-btn btn-approve" onClick={() => updateStatus(user.id, 'approved')}>✓ Aprovar</button>
                        <button className="action-btn btn-reject" onClick={() => updateStatus(user.id, 'rejected')}>✗ Rejeitar</button>
                      </>
                    )}
                    ...
                    <button className="action-btn btn-delete" onClick={() => deleteUser(user.id)}>🗑️</button>
                  </div>
                </td>
              </tr>
            ))}
