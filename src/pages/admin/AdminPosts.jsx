  ...
  return (
    <AdminLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <button className="action-btn btn-primary" onClick={startCreate}>+ Novo Post</button>
      </div>

      {isFormOpen && (
        <div className="admin-card" style={{ marginBottom: 24, borderLeft: '4px solid #d62828' }}>
          <h2 style={{ color: '#0a2a57', fontSize: 17, fontWeight: 700, marginBottom: 20 }}>
            {creating ? '✏️ Criar Novo Post' : '✏️ Editar Post'}
          </h2>
              ...
              <tr key={post.id}>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button className="action-btn btn-edit" onClick={() => startEdit(post)}>✏️ Editar</button>
                    <button className="action-btn btn-delete" onClick={() => handleDelete(post.id)}>🗑️ Excluir</button>
                  </div>
                </td>
              </tr>
            ))}
